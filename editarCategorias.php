...
<head>
  ...
  if(isset($_GET['excluir'])){
    $qt = mysqli_query($conexao,"DELETE FROM `categoriaestoque` WHERE id_CategoriaEstoque=".$_GET['excluir']);
      ...
      <div id="page-content-wrapper">
        ...
        <div class="container-fluid">
          ...
          <table id="tabela" class="table table-hover">
            ...
            $consulta="Select * from categoriaestoque where id_CategoriaEstoque!=6";
            $linhas=mysqli_query($conexao,$consulta);
            $quebra = chr(13).chr(10);
            while($dado=mysqli_fetch_array($linhas)){ ?>
              <tbody>
              <tr>
                <td><?php echo $dado['nomeCategoriaEstoque']; ?></td>
                <td>
                  <a href="alteracaoCategorias.php?alterar=<?php echo $dado['id_CategoriaEstoque']; ?>"><button class="btn btn-outline-success">Editar</button></a>
                  <a href="editarCategorias.php?excluir=<?php echo $dado['id_CategoriaEstoque']; ?>" data-confirm='Tem certeza que deseja excluir?'><button class="btn btn-outline-danger">Excluir</button></a>
